     ...
     "$event, $handler": function(event, handler){
       if(event.type === 'Literal'){
         if(event.value !== 'keydown'){
        		return {failure: "Are you using `'keydown'` as event type?"}
         }
       }else{
       ...
       if(handler.name !== 'up'){
          return {failure: 'Are you setting `up` as your second argument of `.addEventListener()`?'};
       }
     	 return true;
   	 }
   }
   ...
   let onEventCallbacks = {
     "$event, $handler": function(event, handler){      
       if(event.name !== 'onkeydown'){
         ...
         return {failure: "Are you using the `.onkeydown` property to add your event handler?"}
       }
       
       if(handler.name !== 'up'){
         return {failure: "Are you setting `up` as your event handler function?"}
       }
     	 return true;
   	 }   
   }
